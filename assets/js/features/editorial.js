...
function initEditorialScrub() {
  ...
  sections.forEach((section) => {
    ...
    // ResizeObserver debounced for layout updates
    ...
    function renderFrame() {
      ...
      // Exponential lerp smoothing k = 0.02 (scrub: 2.5 equivalent)
      easedProgress += (targetProgress - easedProgress) * 0.02;
      const p = easedProgress;

      // Handle Initial Entry Animation when panel is in top 85%
      if (!hasEntered && rect.top <= viewportH * 0.85) {
        hasEntered = true;
      }

      const entryFactor = hasEntered ? Math.min(1, Math.max(0, (viewportH * 0.85 - rect.top) / (viewportH * 0.3))) : 0;
      ...
      if (n === 1) {
        ...
        // Multi-row timeline scrub calculation
        // Total duration per transition = 3.2 units, hold = 0.8 units
        ...
        // Interpolate stage X between activeIdx and activeIdx + 1
        // power2.inOut curve cubic-bezier(0.45, 0, 0.55, 1) approx:
        const tStage = transProgress < 0.5
          ? 2 * transProgress * transProgress
          : 1 - Math.pow(-2 * transProgress + 2, 2) / 2;

        const currentStageX = stageX[activeIdx] + (stageX[activeIdx + 1] - stageX[activeIdx]) * tStage;

        stage.style.transform = `translate3d(${currentStageX}px, -50%, 0) scale(${entryScale})`;
        stage.style.opacity = `${entryOpacity}`;

        // Cards & Texts transitions
        cards.forEach((c, idx) => {
          if (idx === activeIdx) {
